...
int main ( void )
{
  ...
  int count_max = 1000;
    ...
    char *filename = "mandelbrot_runtime.ppm";
  ...
  double tiempos[2];
  #pragma omp parallel num_threads(omp_get_max_threads()) firstprivate(i,j,x,y,c) shared(n,count,count_max,x_max,x_min,y_max,y_min,c_max,r,g,b,mut,aux)
  {
  #pragma omp single
  {
  timestamp ( );
