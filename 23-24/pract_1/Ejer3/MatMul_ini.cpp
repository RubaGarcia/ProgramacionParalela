...
void Calcula_Maximo_par_out(float *M, int dim, int numHilo);
void Calcula_Maximo_par_in(float *M, int dim, int numHilo);
...
/*
  ...
  Escribir_Matriz (C, dim);*/
	...
	for(auto i = 0; i<NTHREADS; i++){
		threads[i] = std::thread(Calcula_Maximo_par_in, C, dim, i);   
	}
	...
	while(!ultimoIndiceAtendido) {
		//esperamos a que se atienda el ultimo indice
	}
  ...
  printf("El máximo es %f\n", globalMax_in);
	...
	for(auto i = 0; i<NTHREADS; i++){
        ...
        threads[i] = std::thread(Calcula_Maximo_par_out, C, dim, i);   
    }
...
void Calcula_Maximo_par_in(float *M, int dim, int numHilo)
{
	...
	int elementos = dim * dim / NTHREADS;
	//en caso de que no sea divisible la dimension de la matriz al cuadrado por el numero de hilos
	if(numHilo==NTHREADS-1 && dim*dim%NTHREADS!=0){
		elementos += dim*dim%NTHREADS;
	}

    int IndiceInicio = numHilo * elementos;
    int IndiceFinal = (numHilo + 1) * elementos;
...
void Calcula_Maximo_par_out(float *M, int dim, int numHilo)
{
	...
	std::lock_guard<std::mutex> lock(maxMutex);
	{
		if (localGlobal > globalMax_out) {
			globalMax_out = localGlobal;
		}
	}
	ultimoIndiceAtendido = true;
